...
export class Game {
	...
	private readonly MAX_PARTICIPANTS_PER_RACE =
		GAME_CONFIG.MAX_PARTICIPANTS_PER_RACE;
	...
	private initializeGame() {
		...
		this.server.on("connection", (socket) => {
			...
			socket.on("CheckIfRoomIDExists", (roomID) => {
				 ...
				 */
				const USER_JOINED_THE_ROOM_ALREADY = foundRoom.participants.get(
					socket.userID,
				);
		...
		this.server.use((socket, next) => {
			...
			if (auth.userID) {
				...
				if (foundUser) {
					return next(
						new Error(
							"Your account is already connected to our websocket server. Please disconnect your connected account or use a different browser.",
						),
					);
				}
			} else {
