...
const useHandleRooms = ({ session, roomID }: Props) => {
  ...
  const handleRoomIDVerified = React.useCallback(({ roomOwnerID }: SendRoomIDPayload) => {
  ...
  const changeGameStatus = React.useCallback((status: RaceStatus) => {
  ...
  const handlePlayerJoinedOrLeftRoom = React.useCallback((players: Array<ParticipantInformation>) => {
